...
result_path = "result/"
plot_path = "plots_osmd_mabs_vrb_optimal/"
std_plot = 1.0
...
fig.legend(labels=[r'Ada-OSMD with $\alpha=0.4$', r"MABS with $\alpha=0.4$", r"VRB with $\alpha=0.4$", 'Optimal'], loc='lower center', ncol=4, fontsize=25)
fig.savefig(plot_path+'loss_regret_AdaOSMD-MABS-VRB-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
...
for j, sigma in enumerate(sigma_list):
    ...
    axes[j].plot(np.arange(1, n_iter+1), log_loss_list_MABS_mean, color="m", label=r"MABS with $\alpha=0.4$", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_loss_list_MABS_mean - std_plot*log_loss_list_MABS_std, 
                log_loss_list_MABS_mean + std_plot*log_loss_list_MABS_std, facecolor='m', alpha=0.2,
                edgecolor='m', linestyle='dashdot')

    axes[j].plot(np.arange(1, n_iter+1), log_loss_list_VRB_mean, color="c", label=r"VRB with $\alpha=0.4$", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_loss_list_VRB_mean - std_plot*log_loss_list_VRB_std, 
                log_loss_list_VRB_mean + std_plot*log_loss_list_VRB_std, facecolor='c', alpha=0.2,
                edgecolor='c', linestyle='dashdot')
...
fig.legend(labels=[r'Ada-OSMD with $\alpha=0.4$', r"MABS with $\alpha=0.4$", r"VRB with $\alpha=0.4$", 'Optimal'], loc='lower center', ncol=4, fontsize=25)
fig.savefig(plot_path+'loss_AdaOSMD-MABS-VRB-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
...
for j, sigma in enumerate(sigma_list):
    ...
    axes[j].plot(np.arange(1, n_iter+1), log_regret_list_MABS_mean, color="m", label=r"MABS with $\alpha=0.4$", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_regret_list_MABS_mean - std_plot*log_regret_list_MABS_std, 
                log_regret_list_MABS_mean + std_plot*log_regret_list_MABS_std, facecolor='m', alpha=0.2,
                edgecolor='m', linestyle='dashdot')

    axes[j].plot(np.arange(1, n_iter+1), log_regret_list_VRB_mean, color="c", label=r"VRB with $\alpha=0.4$", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_regret_list_VRB_mean - std_plot*log_regret_list_VRB_std, 
                log_regret_list_VRB_mean + std_plot*log_regret_list_VRB_std, facecolor='c', alpha=0.2,
                edgecolor='c', linestyle='dashdot')
...
fig.legend(labels=[r'Ada-OSMD with $\alpha=0.4$', r"MABS with $\alpha=0.4$", r"VRB with $\alpha=0.4$"], loc='lower center', ncol=3, fontsize=25)
fig.savefig(plot_path+'regret_AdaOSMD-MABS-VRB-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
