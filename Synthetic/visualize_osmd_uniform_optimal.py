...
result_path = "result/"
plot_path = "plots_osmd_uniform_optimal/"
std_plot = 1.0
...
for j, sigma in enumerate(sigma_list):
    # load the result
    with open(result_path + "log_loss_list_uniform_mean_sigma=" + str(sigma) + ".txt", "rb") as f:
        log_loss_list_uniform_mean = pickle.load(f)

    with open(result_path +  "log_loss_list_uniform_std_sigma=" + str(sigma) + ".txt", "rb") as f:
        log_loss_list_uniform_std = pickle.load(f)
    ...
    # load the result
    with open(result_path + "log_regret_list_uniform_mean_sigma=" + str(sigma) + ".txt", "rb") as f:
        log_regret_list_uniform_mean = pickle.load(f)

    with open(result_path + "log_regret_list_uniform_std_sigma=" + str(sigma) + ".txt", "rb") as f:
        log_regret_list_uniform_std = pickle.load(f)
    ...
    n_iter = len(log_loss_list_uniform_mean)

    axes[0][j].plot(np.arange(1, n_iter+1), log_loss_list_uniform_mean, color="r", label="Uniform", linewidth=3)
    axes[0][j].fill_between(np.arange(1, n_iter+1), log_loss_list_uniform_mean - std_plot*log_loss_list_uniform_std, 
                log_loss_list_uniform_mean + std_plot*log_loss_list_uniform_std, facecolor='r', alpha=0.2,
    ...
    axes[1][j].plot(np.arange(1, n_iter+1), log_regret_list_uniform_mean, color="r", label="Uniform", linewidth=3)
    axes[1][j].fill_between(np.arange(1, n_iter+1), log_regret_list_uniform_mean - std_plot*log_regret_list_uniform_std, 
                log_regret_list_uniform_mean + std_plot*log_regret_list_uniform_std, facecolor='r', alpha=0.2,
                edgecolor='r', linestyle='dashdot')
...
fig.legend(labels=['Uniform', r'Ada-OSMD with $\alpha=0.4$', 'Optimal'], loc='lower center', ncol=3, fontsize=25)
fig.savefig(plot_path+'loss_regret_AdaOSMD-Uniform-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
...
for j, sigma in enumerate(sigma_list):
    ...
    axes[j].plot(np.arange(1, n_iter+1), log_loss_list_uniform_mean, color="r", label="Uniform", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_loss_list_uniform_mean - std_plot*log_loss_list_uniform_std, 
                log_loss_list_uniform_mean + std_plot*log_loss_list_uniform_std, facecolor='r', alpha=0.2,
                edgecolor='r', linestyle='dashdot')
...
fig.legend(labels=['Uniform', r'Ada-OSMD with $\alpha=0.4$', 'Optimal'], loc='lower center', ncol=3, fontsize=25)
fig.savefig(plot_path+'loss_AdaOSMD-Uniform-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
...
for j, sigma in enumerate(sigma_list):
    ...
    n_iter = len(log_regret_list_uniform_mean)

    axes[j].plot(np.arange(1, n_iter+1), log_regret_list_uniform_mean, color="r", label="Uniform", linewidth=3)
    axes[j].fill_between(np.arange(1, n_iter+1), log_regret_list_uniform_mean - std_plot*log_regret_list_uniform_std, 
                log_regret_list_uniform_mean + std_plot*log_regret_list_uniform_std, facecolor='r', alpha=0.2,
                edgecolor='r', linestyle='dashdot')
...
fig.legend(labels=['Uniform', r'Ada-OSMD with $\alpha=0.4$'], loc='lower center', ncol=2, fontsize=25)
fig.savefig(plot_path+'regret_AdaOSMD-Uniform-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)

# visualiztion only includes uniform and optimal
# plot of loss
fig, axes = plt.subplots(1, 3, figsize=[24., 8.])
...
fig.legend(labels=['Uniform', 'Optimal'], loc='lower center', ncol=2, fontsize=25)
fig.savefig(plot_path+'loss_Uniform-Optimal.png', dpi=400, bbox_inches='tight')
plt.close(fig)
